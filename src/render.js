export default (elements, initialState, i18nInstance) => (path, value, previousValue) => {
  ...
  if (initialState.addingFeedsProcess.valid === false) {
    inputEl.classList.remove('is-valid');
    inputEl.classList.add('is-invalid');
    feedbackEl.classList.remove('text-success');
    feedbackEl.classList.add('text-danger');
    feedbackEl.textContent = initialState.addingFeedsProcess.errors;
    return;
  }
  if (initialState.addingFeedsProcess.valid === true) {
  ...
  if (initialState.loadingProcess.state === 'success') {
    const { posts, feeds } = initialState.loadingProcess;
  ...
  if (initialState.loadingProcess.state === 'failed') {
    feedbackEl.classList.remove('text-success');
    feedbackEl.classList.add('text-danger');
    feedbackEl.textContent = initialState.loadingProcess.errors;
  }
};
